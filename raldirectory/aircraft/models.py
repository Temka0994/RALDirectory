...
class AircraftStatus(models.TextChoices):
    ...
    DESTROYED = "DESTROYED", "Знищений"
    CAPTURED = "CAPTURED", "Захоплений"


class Subdivision(models.Model):
    name = models.CharField(max_length=200, verbose_name="Назва підрозділу")
    military_unit_number = models.CharField(max_length=10, verbose_name="Номер військової частини")
...
class Aircraft(models.Model):
    ...
    serial_number = models.CharField(max_length=20, null=True, blank=True, verbose_name="Серійний номер")
    ...
    def __str__(self):
        return f'{self.model}, {self.tail_number} "{self.tail_color}"'
...
class AircraftPhoto(models.Model):
    ...
    def __str__(self):
        return f'{self.aircraft.model}, {self.aircraft.tail_number} "{self.aircraft.tail_color}"'
